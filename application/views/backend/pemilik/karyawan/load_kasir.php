<table style="width:100%" id="datatable_kasir" class="table table-bordered table-striped">
    <caption></caption>
    <thead>
        <tr>
    ...
    <tbody>
        <?php 
            $no = 1;
            foreach($kasir->result() as $row) {
                if($row->level_karyawan == "Kasir"){
        ?>
        <tr>
...
<script>
    $(document).ready(function() {
        var table = $('#datatable_kasir').DataTable( {
            responsive: true
        });
    });
</script>